...
from .models import Book, ReviewRating, BookGallery
...
def search(request):
    if 'keyword' in request.GET:
        ...
        if keyword:
            books = Book.objects.order_by('-creation_date').filter(
                Q(book_description__icontains=keyword) | Q(book_name__icontains=keyword))
            print(books)
            books_count = books.count()
    context = {
        'books': books,
